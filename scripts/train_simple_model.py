...
Uses only the 5 features available in the frontend interface.
"""
...
def main():
    print("="*80)
    print("TRAINING SIMPLIFIED MODEL - Real NASA Data with Frontend Features Only")
    print("="*80)
    ...
    # Select ONLY the 5 features available in frontend
    print("\n2. Selecting frontend-compatible features...")
    feature_columns = [
    ...
    print(f"   Feature list: {processed_data['feature_columns']}")
    ...
    print("\n6. Registering models in registry...")
    registry = ModelRegistry("models/registry")
    
    for model_name, model in results['models'].items():
        model_id = registry.register_model(
            model=model,
            model_name=f"simple_{model_name.lower().replace(' ', '_')}",
            algorithm=model_name,
            evaluation_results=results['evaluation_results'][model_name],
            hyperparameters={},
            training_info={
                'dataset': 'NASA Kepler KOI (Real Data - Frontend Features Only)',
                ...
                'features_used': '5 frontend features + 6 derived'
            },
            feature_columns=processed_data['feature_columns']
        )
    
    # Show registry summary
    print("\n7. Model Registry Summary:")
    registry.print_registry_summary()
    
    print("\n" + "="*80)
    print("✅ TRAINING COMPLETE!")
    print("="*80)
    print("\nYour simplified model is ready to use!")
    print("This model uses only the 5 features from the frontend interface.")
    print("\nNext steps:")
    print("1. Restart the API server (Ctrl+C and run again):")
    print("   python -m uvicorn api.main:app --reload")
    print("\n2. The API will automatically load the new model")
    print("\n3. Test with the frontend at http://localhost:3000")
    print("="*80)
